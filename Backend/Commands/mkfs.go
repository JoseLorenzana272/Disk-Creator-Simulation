...
func commandMkfs(mkfs *MKFS) error {
	...
	superBlock.PrintUsersFileContent(partitionPath)
	global.GroupsUser[IdPartitionGlobal] = make(map[string][]string)
	global.GroupsUser[IdPartitionGlobal]["root"] = []string{"root"}
	return nil
}
