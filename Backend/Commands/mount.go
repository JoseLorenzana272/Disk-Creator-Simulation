...
func commandMount(mount *MOUNT) error {
	...
	//Verificar que sea una partición primaria
	if partition.Part_type[0] != byte('P') {
		fmt.Println("Error: la partición no es primaria")
		return errors.New("la partición no es primaria")
	}
